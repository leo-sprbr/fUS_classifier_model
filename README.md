...
Functional Ultrasound Imaging (fUS) is a method of brain imaging allowing for very high spatiotemporal resolution of hemodynamic recording in the brain. It works by recording the planar ultrasound echos 
at a very high frequency and through SVD we can form images highlighting the doppler effect of red blood cells, therefore showing a proxy for the cerebral blood flow. In our case, fUS was taken in macaques during
an oculomotor saccade-antisaccade task, during which the monkeys sometimes decided to stop doing the tasking, taking a "pause". This has been researched and analysed in a [previous study](https://doi.org/10.1101/2025.04.01.646546) where a CNN classifier was built and was shown to be able to classify the monkey's behavior (if it was in a state of "work" or "pause") only based on single fUS frames.
