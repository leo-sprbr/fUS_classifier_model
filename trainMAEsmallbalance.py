...
print("ONLY SECUNDO DATA WITHOUT HIGH PASS FILTER ACQ-WISE SPLIT, MASKED CBV, NO SHAVE OFF SIDES, SMALL VIDEO-MAE TEST WITH DROPOUT, WINDOW 16, STRIDE 1 BALANCED DATASET ZSCORE NORM, bALANCE, HIGHPASS TEST")
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.mode == 'train':
                ...
                affine = T.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.90, 1.1))
...
# Model path
best_model_path = f'fus_videomae_small_{window_size}_hp_balanced_zscore_masked_STABLE_balance.pth'
...
plt.tight_layout()
plt.savefig(f'accuracy_plot_STABLE_{window_size}_zscore_hp_balance.png', dpi=300)
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'loss_plot_STABLE_{window_size}_zscore_hp_balance.png', dpi=300)
plt.close()
